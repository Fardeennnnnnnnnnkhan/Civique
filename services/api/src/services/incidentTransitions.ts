...
export async function transitionIncident(tx: Prisma.TransactionClient, input: { incidentId: string; to: IncidentStatus; actorId: string; actorRole: UserRole; actorLabel: string; idempotencyKey?: string; metadata?: Record<string, unknown> }) {
  const commandKey = input.idempotencyKey?.trim();
  if (commandKey) {
    const prior = await tx.outboxEvent.findUnique({ where: { idempotencyKey: `incident-transition:${commandKey}` } });
    if (prior) {
      const replay = await tx.incident.findUnique({ where: { id: input.incidentId } });
  ...
  await tx.outboxEvent.create({ data: { topic: 'incident.status_changed', payload: { incidentId: input.incidentId, previousStatus: incident.status, status: input.to, actorId: input.actorId }, idempotencyKey: commandKey ? `incident-transition:${commandKey}` : `incident:${input.incidentId}:${incident.status}:${input.to}:${input.actorId}:${Date.now()}` } });
